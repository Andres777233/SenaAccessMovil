...
- Credenciales: `admin@sena.edu.co` / `andres.vargas@sena.edu.co` con `12345678`; instructores/aprendices con `123456`.
...
- 2026-08-24 — Preparado para Railway: RetrofitClient añade BASE_URL_REMOTE (HTTPS) como 3er fallback USB->WiFi->Railway; backend WEB con railway.json (nixpacks + migrate --seed) y .gitignore que protege .env. Repos locales commiteados (sin secretos); falta push GitHub (SSH denegado) y deploy Railway (sin CLI/login).
